...
class MetadataSocketServer:
    ...
    def _broadcast_metadata_loop(self):
        ...
        while self._is_broadcasting and not self._thread_stop_event.is_set():
            ...
            for stream, md in all_metadata.items():
                if not md or "point_cloud" not in md:
                    continue
                pc = md["point_cloud"]
                # encode vertices
                if "vertices" in pc and isinstance(pc["vertices"], np.ndarray):
                        raw_bytes = pc["vertices"].tobytes()
                        pc["vertices"] = base64.b64encode(raw_bytes).decode("utf-8")
