        ...
        try:
            while device_id in self.pipelines:
                try:
                    ...
                    # NEW: grabbing both depth & color frames
                        ...
                        for stream_type in self.active_streams[device_id]:
                            ...
                            try:
                                ...
                                if stream_type == rs.stream.depth.name:
                                    ...
                                    frame = rs.colorizer().colorize(frame_data).get_data()  # assuming no throw if 'not frame'
                                ...
                                if points:
                                    # pull raw array
                                    raw = np.asanyarray(points.get_vertices()).view(np.float32).reshape(-1, 3)
                                    
                                    # try a gentler near-clip, or skip it:
                                    filtered = raw[raw[:,2] >= 0.001]
